...
def spsi(
    ...
    *,
    ...
    while i < len(events):
        ...
        _spsi_group(active, np.array([current_mags[m] for m in active]), m_phase, a, fc)
        for m in active:
            phase_out[m][current_frame[m]] = m_phase[m]
...
def _spsi_group(
    active: list[int], sabs: np.ndarray, m_phase: np.ndarray, a: np.ndarray, fc: np.ndarray
) -> None:
    """One time step of SPSI: advance ``m_phase`` of the channels ``active``
    (ascending, i.e. in frequency order), whose magnitudes at this time are
    ``sabs``, in place.  ``a`` are the hops, ``fc`` the centre frequencies
