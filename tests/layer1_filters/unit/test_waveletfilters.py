...
class TestReconstruction:
    ...
    Notes
    -----
    waveletfilters with linspace scales and uniform sampling does NOT form
    a painless frame: the filters don't cover the full positive spectrum
    and the uniform hop violates the painless support condition for the
    wider low-frequency filters.  We therefore test reconstruction with
    cqtfilters and audfilters, which are designed for painless reconstruction.
    """

    def test_reconstruction_cqtfilters(self):
        """CQT filterbank reconstruction via painless dual is bounded.

        The diagonal (painless) dual achieves moderate reconstruction
        error whose magnitude depends on the condition number κ of the
        frame.  For the chosen parameters κ ≈ 64, so we only require
        the error to be < 1.0 (clearly not divergent).
        """
        ...
        f = np.random.RandomState(42).randn(L)
        c = filterbank(f, g, a)
        r = ifilterbank(c, gd, a, real=True)
        rel_err = np.linalg.norm(r[:Ls] - f[:Ls]) / np.linalg.norm(f[:Ls])
        assert rel_err < 1.0, f"CQT reconstruction error: {rel_err}"
